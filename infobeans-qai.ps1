...
if ($runConfigure) {
    Step 1 $totalSteps "Configuring ibtest..."
    Write-Host ""
    ...
    $lmkLines | Set-Content "$ProjectDir\local.mk"
    Ok
}
...
pip install "setuptools==69.5.1" wheel | Out-Null
...
pip install "pytest-dependency==0.6.0" --no-build-isolation
if ($LASTEXITCODE -ne 0) { Fail "Failed to install pytest-dependency."; return }
pip install "$wheelPath" "--prefer-binary"
...
$envLines | Set-Content "$ProjectDir\.env"
Ok
...
$activateLines | Set-Content "$ProjectDir\activate.ps1"
Ok
...
Write-Host "    infobeans-qai run --app $appName   -- run your tests"
Write-Host "    infobeans-qai --help               -- see all commands"
Write-Host ""
Write-Host "  To register with Claude Desktop:"
