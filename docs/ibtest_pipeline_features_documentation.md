...
# ibtest Framework - Pipeline Features Documentation

## CI/CD Pipeline Architecture

This document provides a comprehensive overview of the GitHub Actions features, capabilities, and implementation details within the ibtest framework.
...
*This comprehensive setup guide ensures proper configuration of all external dependencies and secure credential management for the ibtest framework across both GitHub Actions and Azure DevOps environments.*
...
- name: Send email notification
  ...
  with:
    ...
    subject: "ibtest Test Execution updates: ${{ steps.execute_tests.outcome == 'success' && 'PASSED' || 'FAILED' }} at ${{ env.TIMESTAMPFORSUBJECT }}"
    to: ${{ secrets.NOTIFICATION_EMAIL }}
    from: "ibtest Testing Team <${{ secrets.GMAIL_USERNAME }}> "
    html_body: ${{ steps.prepare-email.outputs.email_body }}
    attachments: "${{ env.ZipReportName }},ibtest_test_summary.pdf"
```
...
- name: Execute Tests
  run: |
    if [ "${{ github.event.inputs.mode }}" = "parallel-with-pytest" ]; then
      python -m ibtest -n "$workers" --dist=loadscope
    fi
...
python -m ibtest run --app saucedemo --config config.yaml --workers-count auto --debug-level DEBUG
```
